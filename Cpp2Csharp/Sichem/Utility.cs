...
namespace Sichem
{
	...
	public static class Utility
	{
		...
		public static string ToCSharpTypeString(this CXType type, bool treatArrayAsPointer = false, bool replace = true)
		{
			...
			switch (type.kind)
			{
				...
				case CXTypeKind.CXType_Pointer:
					...
					break;
				default:
					var args = type.NumArgTypes;
					if (args > 0)
					{
						//  spelling=$"({type.ResultType.ToCSharpTypeString()} "
						spelling = clang.getCanonicalType(type).ToPlainTypeString();

						spelling = "CRuntime.Func_" + Convert.ToString(spelling.GetHashCode(), 16) + $" /* delegate=>{spelling}*/";

						//for (uint i=0;i<args;i++)
						//{
						//    var arg = type.GetArgType(i);
		...
		private unsafe static CXChildVisitResult ActionVisitor(CXCursor cursor, CXCursor parent, void* data)
		{
		...
		public static void VisitWithAction(this CXCursor cursor, Func<CXCursor, CXChildVisitResult> func)
		{
			...
			unsafe
			{
				cursor.VisitChildren(ActionVisitor, new CXClientData(IntPtr.Zero));
		...
		public static string[] Tokenize(this CXCursor cursor, CXTranslationUnit translationUnit)
		{
			...
			var nativeTokens = translationUnit.Tokenize(range);

			var result = new List<string>();
			var tokens = nativeTokens.ToArray(); ;// new CXToken[nativeTokens.Length];
			for (uint i = 0; i < tokens.Length; ++i)
			{
				//tokens[i] = (CXToken)Marshal.PtrToStructure(nativeTokens, typeof(CXToken));
				//nativeTokens += Marshal.SizeOf(typeof(CXToken));

				var name = tokens[i].GetSpelling(translationUnit).CString;// clang.getTokenSpelling(translationUnit, tokens[i]).ToString();
				result.Add(name);
			}
