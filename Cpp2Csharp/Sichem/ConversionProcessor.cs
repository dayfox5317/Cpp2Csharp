...
namespace Sichem
{
	internal class ConversionProcessor : BaseProcessor
	{
		...
		private class Ctor_Struct
		{
			public string Prefix = "public";
			public List<string> Names = new List<string>();
			public List<string> Params = new List<string>();
			public List<string> Params2 = new List<string>();
		...
		private Dictionary<string, Ctor_Struct> Temp_Ctor = new Dictionary<string, Ctor_Struct>();
		private unsafe CXChildVisitResult VisitStructs(CXCursor cursor, CXCursor parent, void* data)
		{
			...
			switch (curKind)
			{
				...
				case CXCursorKind.CXCursor_StructDecl:
					...
					if (string.IsNullOrEmpty(structName))
					{
						...
						clang.getCursorSemanticParent(cursor).VisitChildren(forwardDeclaringVisitor.Visit,
					...
					if (!_visitedStructs.Contains(structName) && cursor.GetChildrenCount() > 0)
					{
						...
						if (!Temp_Ctor.ContainsKey(_structName))
						{
							Temp_Ctor[_structName] = new Ctor_Struct() { Name = _structName };
						}
						cursor.VisitChildren(VisitStructs, new CXClientData(IntPtr.Zero));

						var stw = Temp_Ctor[_structName]?.GetString();
						IndentedWriteLine(stw);
				...
				case CXCursorKind.CXCursor_FieldDecl:
					...
					Temp_Ctor[_structName]?.Names.Add(fieldName);
					...
					if (!_parameters.GenerateSafeCode)
					{
							...
							expr.Info.Type.GetPointeeType().kind != CXTypeKind.CXType_Record)
						{
							...
							Temp_Ctor[_structName]?.Params2.Add($"Marshal.Copy({fieldName}, 0, new IntPtr(this.{fieldName}), {fieldName}.Length);");
						...
						else
						{
							...
							Temp_Ctor[_structName]?.Params.Add(expr.Info.CsType + " " + fieldName);
		...
		private unsafe CXChildVisitResult VisitEnums(CXCursor cursor, CXCursor parent, void* data)
		{
			...
			if (curKind == CXCursorKind.CXCursor_EnumConstantDecl)
			{
				var _enumName2 = clang.getCursorSpelling(cursor).ToString().Trim();
			...
			if (curKind == CXCursorKind.CXCursor_EnumDecl)
			{
				...
				{
					//    _enumName = Guid.NewGuid().ToString();
				}
			...
			return CXChildVisitResult.CXChildVisit_Recurse;//fixed if is continue ,will loss some 
		}

		int cnt = 0;

		private unsafe CXChildVisitResult VisitGlobalVariables(CXCursor cursor, CXCursor parent, void* data)
		{
			if (cursor.IsInSystemHeader())
			{
				return CXChildVisitResult.CXChildVisit_Continue;
		...
		private unsafe CXChildVisitResult VisitFunctionsPreprocess(CXCursor cursor, CXCursor parent, void* data)
		{
			if (cursor.IsInSystemHeader())
			{
				return CXChildVisitResult.CXChildVisit_Continue;
		...
		private unsafe CXChildVisitResult VisitFunctions(CXCursor cursor, CXCursor parent, void* data)
		{
			if (cursor.IsInSystemHeader())
			{
				return CXChildVisitResult.CXChildVisit_Continue;
		...
		private string InternalProcess(CursorInfo info)
		{
			...
			switch (info.Kind)
			{
				...
				case CXCursorKind.CXCursor_CallExpr:
					{
						var size = info.Cursor.GetChildrenCount();
						if (info.Cursor.CompletionString.ToString().Contains("STBRP_ASSERT"))
						{

						}
						if (info.Cursor.Spelling.CString.Contains("STBRP_ASSERT"))
						{
						...
						for (var i = 1; i < size; ++i)
						{
							...
							if (argExpr.Expression.IndexOf("\"") != argExpr.Expression.LastIndexOf("\""))
							{
								var hash = argExpr.Expression.GetHashCode();
								var gc = argExpr.Info.IsPointer ? "_sbyte" : "_string";
								var str = $"CRuntime.GetStr{gc}(table_{Convert.ToString(hash, 16)}) /*{argExpr.Expression}*/";
								args.Add(str);
								continue;
							}
							...
							if (functionInfo != null && functionInfo.ArgumentTypes.Count >= (i) && functionInfo.ArgumentTypes[i - 1] == FunctionArgumentType.Ref)
							{
		...
		private CursorProcessResult Process(CXCursor cursor)
		{
			...
			if (!string.IsNullOrEmpty(expr))
			{
				if (expr.Contains("!"))
				{
					// if (expr.Contains("!"))
					{
						var indef = expr.IndexOf("!!");
						if (indef != -1)
							expr = expr.Remove(indef, 2);
					}
				}
			}
		...
		private unsafe CXChildVisitResult VisitFunctionBody(CXCursor cursor, CXCursor parent, void* data)
		{
			var res = Process(cursor);
		...
		private void ProcessFunctionPreprocess(CXCursor cursor)
		{
			...
			//params <= ...
			if (functionType.IsFunctionTypeVariadic)
			{
				var t = FunctionArgumentType.Default;
				if (numArgTypes > 0)
				{
		...
		private unsafe void ProcessFunction(CXCursor cursor, bool is_static = true)
		{
			WriteFunctionStart(cursor);

			_indentLevel++;

			_functionStatement.VisitChildren(VisitFunctionBody, new CXClientData(IntPtr.Zero));
		...
		public unsafe override void Run()
		{
			...
			if ((_parameters.SkipFlags & SkipFlags.Enums) != SkipFlags.Enums)
			{
				...
				clang.getTranslationUnitCursor(_translationUnit).VisitChildren(VisitEnums, new CXClientData(IntPtr.Zero));
				//  clang.getTranslationUnitCursor(_translationUnit).VisitChildren(VisitEnums, new CXClientData(IntPtr.Zero));
			...
			if ((_parameters.SkipFlags & SkipFlags.GlobalVariables) != SkipFlags.GlobalVariables)
			{
				...
				clang.getTranslationUnitCursor(_translationUnit).VisitChildren(VisitGlobalVariables,
					new CXClientData(IntPtr.Zero));
			}

			_state = State.Structs;
			Source = "Structs.cs";
			clang.getTranslationUnitCursor(_translationUnit).VisitChildren(VisitStructs, new CXClientData(IntPtr.Zero));

			_state = State.Functions;
			Source = "Functions.cs";
			clang.getTranslationUnitCursor(_translationUnit).VisitChildren(VisitFunctionsPreprocess,
							new CXClientData(IntPtr.Zero));
			clang.getTranslationUnitCursor(_translationUnit).VisitChildren(VisitFunctions, new CXClientData(IntPtr.Zero));
